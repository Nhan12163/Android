...
class MainActivity : AppCompatActivity() {
    ...
    private fun dk(){
        ...
        if(email.isEmpty() || name.isEmpty() || pass.isEmpty() || passrp.isEmpty()  ){
            Toast.makeText(applicationContext, "Điền đầy đủ thông tin", Toast.LENGTH_SHORT).show()
        }else{
            if (pass!=passrp){
                Toast.makeText(applicationContext, "Mật khẩu không giống nhau", Toast.LENGTH_SHORT).show()
            }else{
                db!!.addLogin(email, name, mahoa(pass, key))
